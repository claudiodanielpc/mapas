...
#df = df[df["delito"] == "NARCOMENUDEO POSESIÓN CON FINES DE VENTA, COMERCIO Y SUMINISTRO"]
...
# Capa entidad
cdmx.plot( ax = ax,color = "#1C00ff00",
           edgecolor = "grey",
           linewidth = 0.8 )
# Capa ageb
ageb.plot( ax = ax,color = "#1C00ff00",
           edgecolor = "grey",
           linewidth = 0.1 )
# Capa puntos

ax.scatter( df["longitud"],df["latitud"],
            alpha = 0.6,color = "#756bb1" )
ax.axis( 'off' )
...
# plt.show()
# Salvar
plt.savefig( "cdmxfeminicidio.png",format = "png",dpi = 600,transparent = False )
